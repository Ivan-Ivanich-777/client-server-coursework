...
MainWindow::MainWindow(QWidget *p) : QMainWindow(p) {
    setWindowTitle("Math Solver"); resize(1200, 800);
    m_stack = new QStackedWidget(this); setCentralWidget(m_stack);
    m_title = new TitleForm(); m_task = new TaskForm(); m_dash = new DashboardForm(); m_auth = new AuthForm();
    m_stack->addWidget(m_title); m_stack->addWidget(m_task); m_stack->addWidget(m_dash); m_stack->setCurrentIndex(0);
    m_auth->setParent(this); m_auth->setGeometry((width()-360)/2, (height()-420)/2, 360, 420); m_auth->show();
